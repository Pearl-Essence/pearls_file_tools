...
class _ReviewPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        self.info_label.setText(
            f"{len(include)} file(s) will be included ({_fmt_size(total_incl)})  |  {len(exclude)} file(s) excluded"
        )
...
class _ExportWatcherPane(QWidget):
    ...
    def _start(self):
        ...
        self._log(
            f"[{ts}]  Started watching: {directory.name}  ({len(self._known_files)} existing file(s) ignored)",
            "#c3e88d",
        )
